...
func (e *Extractor) Run() (*Stats, error) {
	...
	log.Info("Pass 1 complete", "nodes", nodeCount, "duration", time.Since(start).Round(time.Second))
	...
	log.Info("Pass 2 complete", "ways", wayCount, "relations", relCount, "duration", time.Since(start).Round(time.Second))
	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-ticker.C:
				log.Debug("Node indexing progress", "nodes", count.Load())
			}
		}
	}()
